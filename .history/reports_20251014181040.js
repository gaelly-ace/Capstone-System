...
/* ====== NEW: No-flash navigation guard for sidebar ====== */
/* 1) Prevent reload if you click the link to the CURRENT page (e.g., Reports on reports.html). */
/* 2) Lock the paint to the correct bg just before navigating to other pages. */
/* 3) Debounce to ignore spam clicks that would chain navigations. */
...
document.querySelectorAll('#sidebar .side-menu.top li a').forEach(a => {
  a.addEventListener('click', (e) => {
    ...
    // If it's the same page, do nothing (prevents reload flash)
    if (same) {
      e.preventDefault();
      return;
    }

    // Debounce spam clicking (first click goes through)
    if (navDebounce) {
      e.preventDefault();
      return;
    }
    ...
    // Lock correct paint before leaving
    paintLock();
    // Allow the browser to process paint lock then navigate
    requestAnimationFrame(() => { window.location.href = a.href; });
    // Prevent default so we control timing
    e.preventDefault();
  });
});
