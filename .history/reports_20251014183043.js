...
/* ===== Theme (no-flash, uses 'dark-mode'); DO NOT touch checkbox state ===== */
...
function applyTheme(dark) {
  ...
  // IMPORTANT: do NOT set switchMode.checked here (per your request)
}

// initial theme from storage (doesn't change checkbox)
applyTheme(localStorage.getItem('dark-mode') === 'true');

// toggle without transitions; checkbox state is changed only by the user click
if (switchMode) {
...
/* ====== No-flash navigation guard for sidebar ====== */
...
document.querySelectorAll('#sidebar .side-menu.top li a').forEach(a => {
  a.addEventListener('click', (e) => {
    ...
    if (same) { // prevent reload on same page
      e.preventDefault();
      return;
    }

    if (navDebounce) { // debounce spam clicks
      e.preventDefault();
      return;
    }
    navDebounce = true;
