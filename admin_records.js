...
let originalData = [
    ...
    {
        livestockId: "LV-2024-009",
        kindOfLivestock: "Cattle",
        gender: "Female",
        weight: 295.6,
        dealer: "Roberto Silva",
        status: "In Slaughter House",
        createdDate: new Date('2024-10-20T13:25:00').toISOString(),
        lastModified: new Date('2024-10-20T13:25:00').toISOString()
    },
    {
        ...
        gender: "Male",
        weight: 95.7,
        dealer: "Elena Martinez",
        status: "In Exit Point",
        createdDate: new Date('2024-10-21T10:10:00').toISOString(),
        lastModified: new Date('2024-10-21T10:10:00').toISOString()
    }
];

let getData = [...originalData];

console.log('Initial data loaded:', originalData.length, 'records');
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log('DOM elements found:', {
        registerBtn: !!registerLivestockBtn,
        userInfo: !!userInfo,
        table: !!table
    });
    ...
    if (qrScanBtn) {
        qrScanBtn.addEventListener('click', function() {
            alert('QR Scanner feature: Please scan a QR code.\n\nFor demo purposes, a sample ID will be generated.');
            const sampleId = 'LV-2024-' + String(Math.floor(Math.random() * 9000) + 1000);
            scannedId = sampleId;
            livestockIdValue.textContent = sampleId;
            document.getElementById('livestockId').value = sampleId;
        });
    }
    ...
    function updateWeightDisplay() {
        if (weightValue) weightValue.textContent = currentWeight.toFixed(1);
    ...
    function resetForm() {
        ...
        selectedGender = '';
        scannedId = '';
        if (weightValue) weightValue.textContent = '0.0';
        if (weightInput) weightInput.value = '';
        if (weightInputField) weightInputField.value = '';
        if (genderInput) genderInput.value = '';
        if (livestockIdValue) livestockIdValue.textContent = 'Not Scanned';
        if (document.getElementById('livestockId')) document.getElementById('livestockId').value = '';
        if (document.getElementById('dealer')) document.getElementById('dealer').value = '';
        genderOptions.forEach(opt => opt.classList.remove('selected'));
    }
    ...
    function preLoadCalculations() {
        ...
        console.log('Calculations:', { arrayLength, maxIndex, tableSize });
    }
    ...
    function showInfo() {
        ...
        console.log('Showing records from', tab_start, 'to', tab_end);
        ...
        if (getData.length > 0) {
            ...
            console.log('Displayed', (tab_end - tab_start), 'records');
        } else {
    ...
    function readInfo(livestockId) {
        ...
        if (livestockIdValue) livestockIdValue.textContent = record.livestockId;
        if (document.getElementById('livestockId')) document.getElementById('livestockId').value = record.livestockId;
        if (document.getElementById("kindOfLivestock")) document.getElementById("kindOfLivestock").value = record.kindOfLivestock;
        if (document.getElementById("dealer")) document.getElementById("dealer").value = record.dealer;
        ...
        if (document.getElementById("status")) document.getElementById("status").value = record.status;
        ...
        if (modalTitle) modalTitle.innerHTML = "Edit Livestock Record";
    ...
    if (registerLivestockBtn) {
        registerLivestockBtn.addEventListener('click', () => {
            ...
            if (modalTitle) modalTitle.innerHTML = "Register New Livestock";
            if (popupFooter) popupFooter.style.display = "flex";
            if (darkBg) darkBg.classList.add('active');
            if (popupForm) popupForm.classList.add('active');
            resetForm();
    ...
    // Initialize the page
    console.log('Initializing page with', originalData.length, 'records');
    preLoadCalculations();
    highlightIndexBtn();
    displayIndexBtn();
